...
const actions = ['Idle', 'Blink', 'Listen', 'Think', 'Speak', 'Wave', 'Celebrate', 'Error', 'Sleep', 'Bounce', 'Spin', 'Shy', 'Mischief'];
const states = ['idle', 'listening', 'thinking', 'speaking', 'success', 'error', 'sleeping'];
const bridge = createPetBridge(Number(process.env.ORANGE_PET_BRIDGE_PORT || 8765));
function result(data) {
    return { content: [{ type: 'text', text: JSON.stringify(data) }], structuredContent: data };
}
function delivery(command) {
...
for (const signal of ['SIGINT', 'SIGTERM'])
    process.on(signal, () => { void bridge.close().finally(() => handle.close()); });
